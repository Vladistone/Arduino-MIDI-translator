...
Arduino map layouts are very custom, so I'm no sure yet if it would make much sense to create map presets of code... but the fundamental approach to the system for developing code for Arduino MIDI translator and consistency in adapting MIDI implementation has already been launched and this requires publicity
*show must go on*...
