...
Arduino map layouts are very custom, so I'm no sure yet if it would make much sense to create presets on map... but show must go on...

The project start from 5th July 2016 and based on a hack https://hackaday.io/project/12541-korg-dw-8000-midi-enhancer[Korg DW8000 midi enhancer] by Steve Baines https://github.com/sjbaines/Korg_DW-8000_Midi_Enhancer[and his code project]: Its code was tested on the Korg DW8000 and may have been stable, but by the time November 2023, when I discovered this project on the network - I could not immediately run it on the EX8000.
It turned out to be non-working and required debugging fixes for DW/EX were added for the pull request, but I did not find any activity from the project's mastermind. Therefore, I independently continued the path of adapting the project for Korg DSS-1 and thanks to mentoring from experienced Arduino coders, I achieved the first results literally a week after the analysis
