...
----
202 void handleControlChange(byte channel, byte number, byte value)
203 {
204  if (channel != dwChannel)
205  { // If CC not sent to DW-8000 channel, then just pass them on
206    MIDI.sendControlChange(number, value, channel);
207    return;
208  }
----
====
...
[,cpp,%linenums]
----
360 void handleControlChange(byte channel, byte number, byte value)
361 {
362  if (channel != dssChannel + 1)
363  {// If CC not sent to DSS-1 channel, then just pass them on
364	MIDI.sendControlChange(number, value, channel);
365	return;
366  }
----
...
- he prefers an independent control option for the `OSC 1 & 2` parameters separetly with the ability to raise the level of both oscillators up to 100% "fat",
while training the limit of powerful synth headroom of output. But it would be increase up to 127% with external Sysex commands (why produser didn't it itself?):
Or is have a reasons?
|===
...
My reason was to create authenticity to
  ...
  * the ability to economize the fader on the SSL Nucleus controller.
