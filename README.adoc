...
. Make sure your `MIDI_library.h` is enabled in kbd:[*Sketch*] → kbd:[*Include Library*] → kbd:[*Manage Librarys*] `⇧⌘i`. if you need kbd:[*download*] or kbd:[*upgrade*] the `MIDI_library.h` - do it!
  * For the Arduino board device, tick for your `device name` in kbd:[*tool*] → kbd:[*device*] → from avalable devices list
  * and tick your *Port* for connection in kbd:[*tool*] → kbd:[*Port*] → from avalable port list
...
Relax! or not? wait a minute...

because after downloading the sketch to the Arduino,
. The time will come to adapt the Arduino to your MIDI system and decide whether this Cntrl_2_Synth map layouts
   * is this convenient for your combination of controller and Korg DSS-1 or whether need to remap the code?
   * You can also check the functionality of this sketch in working conditions or by simulating MIDI monitoring.
. To do this, you need to have a separate MIDI interface and a shield to connect to the Arduino MIDI interface.
. or
open your synthesizer and take a soldering iron and wire cutters in your hands to carry out a surgical operation to insert a chip into the body of the synthesizer!
For this purpose, there is no need for me to describe all the details of the analysis of the code’s performance under working conditions on this site.
Or you can ask a question in a discussion on the forum https://gearspace.com/board/electronic-music-instruments-and-electronic-music-production/1418944-korg-dss-1-arduino-upgrade-project-2.html[gearspace.com]
...
but the base approach of the system for developing code for `Arduino MIDI translator` and consistency in adapting MIDI implementation has already been launched and this requires publicity...
*show must go on*!
...
So therefore, I independently continued the path of adapting the project for Korg DSS-1, thanks to mentoring from experienced Arduino C++ coder. I achieved the first results literally a week after the analysis differences between DW8000 vs DSS-1 of MIDI <<implementation>> charts  with <<troubleshooting>>.
...
which is available on the https://glenstegner.com/dss1/home.html[Glen Stegner]and this site author very deeply described all the possibilities in the work of DSS-1.
...
====
image:IMAGES/COM_IMG/GlenSiegner.png[Glen Stegner,,]
...
[#Unit]
=== Discribtion coding unit:
Unfortunately, I’m not a professional coder and apparently you noticed my lack of professionalism even earlier. Therefore,
I describe the process of creating code as a self-taught person. having experience in coding in basic and fortran when computer centers were relevant and a personal computer was like a spaceship - unattainable. And the current Windows wasn't even 3.1
...
==== Not realized ideas, and parts in progress:
...
Therefore, any help and cooperation is accepted! image:IMAGES/Korg DSS-1/MODE_data.jpeg[,400.500]
====
...
[NOTE]
----
within parentheses wouldn't work if use off/on SW
----
My Arduino project had limitations when designing the lookup table/template: *Nucleus2 SSL controller*
...
[WARNING]
pay attantion: not use the *param.Offset*! parameter misconception is passed down!

the author of the first DW8000 hack didn't understand the difference also
...
which is indicated on the surface and LCD of the synthesizer and was ignored by the developers in previous versions of the code and Excel tables from Glen Stegner.
...
image:IMAGES/Korg DSS-1/LSB_MSB_data_format.jpeg
